...
<xsl:template match="/">
    <html>
        <head>
            <title>MediaBox 视频流状态</title>
        </head>
        <body>
            <xsl:apply-templates select="rtmp"/>
            <hr/>
            Generated by MediaBox <xsl:value-of select="/rtmp/nginx_rtmp_version"/>,
            Compiled on <xsl:value-of select="/rtmp/built"/>&#160;<xsl:value-of select="/rtmp/compiler"/>
        </body>
    </html>
</xsl:template>
...
<xsl:template match="rtmp">
    <table cellspacing="1" cellpadding="5">
        ...
        <tr>
            <td colspan="2">视频流: <xsl:value-of select="naccepted"/></td>
            <th bgcolor="#999999">编码格式</th>
            <th bgcolor="#999999">码率</th>
            <th bgcolor="#999999">分辨率</th>
            <th bgcolor="#999999">FPS</th>
            <th bgcolor="#999999">编码格式</th>
            <th bgcolor="#999999">码率/s</th>
            <th bgcolor="#999999">采样频率</th>
            <th bgcolor="#999999">通道</th>
            <td>
...
<xsl:template name="streamstate">
    <xsl:choose>
        <xsl:when test="active">激活</xsl:when>
...
<xsl:template name="clientstate">
    <xsl:choose>
        <xsl:when test="publishing">推流中</xsl:when>
